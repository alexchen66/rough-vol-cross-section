"""
Run all-windows daily backtest and plots for the no-rough ablation experiment.

Charts are written to two new folders under data/figures:
  data/figures/no_rough_by_year
  data/figures/no_rough_full_period
"""
...
YEARLY_FIGURES = ROOT / "data" / "figures" / "no_rough_by_year"
FULL_FIGURES = ROOT / "data" / "figures" / "no_rough_full_period"

PREDICTIONS_PATH = DATA_PROCESSED / "predictions_no_rough.parquet"
RETURNS_PATH = REPORTS / "portfolio_daily_returns_no_rough_all_windows.csv"


def load_or_run_no_rough(force: bool = True) -> pd.DataFrame:
    if force or not RETURNS_PATH.exists():
        if not PREDICTIONS_PATH.exists():
            raise FileNotFoundError(
                f"Missing {PREDICTIONS_PATH}. Run python src/train_no_rough.py first."
            )
...
def plot_yearly_no_rough(data: pd.DataFrame, market_daily: pd.Series) -> list[Path]:
    outputs = []
    for year in sorted(data["year"].unique()):
        ...
        for model in sorted(year_data["model"].unique()):
            for portfolio in ["long_only", "long_short"]:
                ...
                title = f"{model} no-rough: {year} daily cumulative {portfolio} returns by fee vs market"
                outputs.append(_plot_curves(year_data, market_daily, model, portfolio, out, title))
    return outputs


def plot_full_period_no_rough(data: pd.DataFrame, market_daily: pd.Series) -> list[Path]:
    outputs = []
    for model in sorted(data["model"].unique()):
        for portfolio in ["long_only", "long_short"]:
            ...
            title = f"{model} no-rough: 2016-2023 daily cumulative {portfolio} returns by fee vs market"
            outputs.append(_plot_curves(data, market_daily, model, portfolio, out, title))
    return outputs


def make_no_rough_backtest_summary(data: pd.DataFrame) -> pd.DataFrame:
    rows = []
    ...
    summary.to_csv(REPORTS / "daily_backtest_summary_no_rough_all_windows_by_year.csv", index=False)
    return summary


def make_no_rough_ic_summary() -> pd.DataFrame:
    predictions = pd.read_parquet(PREDICTIONS_PATH)
    ...
    daily_ic.to_csv(REPORTS / "ic_by_rebalance_date_no_rough_all_windows.csv", index=False)
    ...
    summary.to_csv(REPORTS / "ic_summary_no_rough_all_windows_by_year.csv", index=False)
    return summary


def main() -> None:
    data = load_or_run_no_rough(force=True)
    start, end = data["hold_date"].min(), data["hold_date"].max()
    market_daily = load_market_proxy(start, end)

    yearly_outputs = plot_yearly_no_rough(data, market_daily)
    full_outputs = plot_full_period_no_rough(data, market_daily)
    summary = make_no_rough_backtest_summary(data)
    ic_summary = make_no_rough_ic_summary()

    print(f"Saved no-rough daily returns: {RETURNS_PATH}")
    ...
    print(f"Saved no-rough backtest summary rows: {len(summary)}")
    print(f"Saved no-rough IC summary rows: {len(ic_summary)}")
