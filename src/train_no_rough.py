"""
Train four models on the same walk-forward windows without rough-volatility features.

This is an ablation experiment. It leaves the original prediction files intact and writes:
  data/processed/predictions_no_rough.parquet
"""
...
def get_no_rough_feature_cols(df: pd.DataFrame) -> list[str]:
    ...
    return [
        ...
        if col not in rough and col not in rough_missing
    ]


def run_no_rough_walk_forward(
    ...
    feature_cols = get_no_rough_feature_cols(data)
    trading_dates = pd.DatetimeIndex(sorted(data["date"].unique()))
    all_predictions = []

    print(f"Using {len(feature_cols)} no-rough features.")

    for i, (tr_s, tr_e, va_s, va_e, te_s, te_e) in enumerate(WALK_FORWARD_WINDOWS):
        print(
            f"\nNo-rough window {i + 1}/{len(WALK_FORWARD_WINDOWS)}: "
...
def main() -> None:
    ...
    print("Running no-rough walk-forward training...")
    predictions = run_no_rough_walk_forward(features, labels)

    out = DATA_PROCESSED / "predictions_no_rough.parquet"
